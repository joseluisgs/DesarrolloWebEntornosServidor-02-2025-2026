...
public class UserRemoteRepositoryImpl implements UserRemoteRepository {
    ...
    public CompletableFuture<Void> deleteUserCompletableFuture(int id) {
        return userApiRest.deleteUser(id)
                .thenAccept(response -> {
                    // No hace falta hacer nada aquí, ya que el método retorna un CompletableFuture<Void>
                })
                .exceptionally(e -> {
                    if (e.getCause() instanceof retrofit2.HttpException && ((retrofit2.HttpException) e.getCause()).code() == 404) {
                        throw new UserNotFoundException("Usuario no encontrado con id: " + id);
    ...
    public Single<Void> deleteUserWithRxJava(int id) {
        return userApiRest.deleteUserWithRxJava(id)
                .subscribeOn(Schedulers.io())
                // Convertimos Single<Response> a Single<Void>
